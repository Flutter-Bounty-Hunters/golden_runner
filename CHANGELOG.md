## 0.2.3
 * Feature: Add a `--ubuntu-version` option to base golden_runner's built-in image on a specific Ubuntu version (any Docker Hub `ubuntu` tag, e.g. `24.04` or `noble`), without writing a custom Dockerfile. Defaults to `ubuntu:latest`. Useful when golden output depends on the OS's font rendering, or to match a CI runner. Ignored when a custom Dockerfile is provided.
